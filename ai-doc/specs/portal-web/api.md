...
| 分组 | 前缀 | 鉴权 | 用途 |
|---|---|---|---|
| Public API | `/api/v1/auth/*` | 无 / 登录前 | 官网、套餐展示、注册、登录、Admin 登录 |
| Member API | `/api/v1/user/*` | 用户登录 | Profile、规则、设备、日志、统计、账单、团队 |
| Admin API(Sanctum) | `/api/v1/admin/*`(子组) | Sanctum + `permission:admin.access` | 用户、设备、告警、团队、审计日志(总后台查看类) |
| Agent API | `/api/v1/node/*` | `node.hmac`(Bearer + HMAC) | resolver 心跳、拉配置、ACK、查询日志批量 |
...
```http
GET /api/v1/user/dashboard?profile_id=prf_01H...&range=24h
```
...
```http
GET /api/v1/user/profiles?page=1&per_page=20
```
...
> 当前实现采用扁平路径(`/api/v1/user/security`)+ 当前用户默认 Profile 上下文(原 §4.6A 文档要求 `/profiles/{profile_id}/security` 嵌套路径);若后续需要多 Profile 切换,可加 `?profile_id=` query 参数。后续 V2 可扩展为嵌套路径。
...
> 当前实现采用扁平路径(`/api/v1/user/privacy`)+ 当前用户默认 Profile 上下文,语义同 §4.6A。
...
PUT /api/v1/user/parental
```

> 当前实现采用扁平路径(`/api/v1/user/parental`)+ 当前用户默认 Profile 上下文,语义同 §4.6A。
...
```http
GET /api/v1/user/profiles/{profile_id}/rules?type=block&keyword=ads&page=1
```
...
```http
POST /api/v1/user/profiles/{profile_id}/rules
```
...
| `action` | allow list 固定 `allow`，block list 固定 `block` |
...
```http
DELETE /api/v1/user/profiles/{profile_id}/rules/{rule_id}
```
...
```http
GET    /api/v1/user/profiles/{profile_id}/allowlist
POST   /api/v1/user/profiles/{profile_id}/allowlist
DELETE /api/v1/user/profiles/{profile_id}/allowlist/{rule_id}
```
...
```http
GET    /api/v1/user/profiles/{profile_id}/blocklist
POST   /api/v1/user/profiles/{profile_id}/blocklist
PUT    /api/v1/user/profiles/{profile_id}/blocklist/{rule_id}
DELETE /api/v1/user/profiles/{profile_id}/blocklist/{rule_id}
```

POST 请求固定语义：`list_type=block`、`action=block`。
...
```http
POST /api/v1/user/profiles/{profile_id}/rules/import
GET  /api/v1/user/profiles/{profile_id}/rules/export
```
...
GET 请求参数：
- `list_type`: `allowlist` / `blocklist` / `all`（默认 all）
- `format`: `txt` / `json` / `csv`（默认 txt）

### 4.9D Profile 设置（Profile 级）

```http
GET    /api/v1/user/profiles/{profile_id}/settings
PUT    /api/v1/user/profiles/{profile_id}/settings
GET    /api/v1/user/profiles/{profile_id}/settings/security
PUT    /api/v1/user/profiles/{profile_id}/settings/security
GET    /api/v1/user/profiles/{profile_id}/settings/privacy
PUT    /api/v1/user/profiles/{profile_id}/settings/privacy
GET    /api/v1/user/profiles/{profile_id}/settings/parental
PUT    /api/v1/user/profiles/{profile_id}/settings/parental
```
...
```http
GET /api/v1/user/profiles/{profile_id}/publishes/{publish_id}
```
...
```http
GET /api/v1/user/profiles/{profile_id}/devices
```

### 4.13 创建设备

```http
POST /api/v1/user/profiles/{profile_id}/devices
```
...
```http
GET /api/v1/user/profiles/{profile_id}/logs?from=2026-06-12T00:00:00Z&to=2026-06-12T23:59:59Z&domain=example.com&action=blocked&page=1&per_page=50
```
...
```http
GET /api/v1/user/profiles/{profile_id}/stats/summary?range=24h
GET /api/v1/user/profiles/{profile_id}/stats/top-domains?range=24h&action=blocked
GET /api/v1/user/profiles/{profile_id}/stats/timeseries?range=7d&interval=1h
```
...
```http
GET /api/v1/user/profiles/{profile_id}/settings/dns
```
...
```http
GET /api/v1/user/teams/{team_id}/members
```
...
```http
POST /api/v1/user/teams/{team_id}/invitations
```
...
```http
GET /api/v1/user/teams/{team_id}/invitations
```

### 4.27 撤销邀请

```http
DELETE /api/v1/user/teams/{team_id}/invitations/{invitation_id}
```

### 4.28 接受邀请

```http
POST /api/v1/user/teams/accept-invitation
```
...
```http
DELETE /api/v1/user/teams/{team_id}/members/{user_id}
```
...
```http
POST /api/v1/user/teams/{team_id}/switch
```
...
```http
GET /api/v1/user/teams/{team_id}/profiles
```
...
GET    /api/v1/user/api-keys/{key_id}
PUT    /api/v1/user/api-keys/{key_id}
DELETE /api/v1/user/api-keys/{key_id}
POST   /api/v1/user/api-keys/{key_id}/rotate
```
...
GET    /api/v1/user/devices/{device_id}
POST   /api/v1/user/devices/{device_id}/disable
POST   /api/v1/user/devices/{device_id}/enable
```
...
GET  /api/v1/user/invoices/{invoice_id}
...
### 8.4 支付 Provider Webhook

```http
POST /api/v1/auth/billing/webhooks/{provider}
```
...
### 8.5 portal-web(原 console 域) 派生 usage（合并后：进程内服务）
...
dns-resolver 批量上报 query logs
  → portal-web(原 console 域) POST /api/v1/node/dns-resolver/query-logs（已落地）
...
```text
POST /api/v1/user/usage-overage/checkout     禁止
