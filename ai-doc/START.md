...
| 15 | `deploy/*` | 生成本地启动和部署配置 |

## 2.1 API 路径约定（已锁定）

```text
Public API:    /api/v1/public/*      ← 无鉴权，注册、登录、管理员登录
User API:      /api/v1/user/*       ← 用户登录（Sanctum Token），会员中心
Admin API:     /api/v1/admin/*      ← 管理员登录（Sanctum Token），后台管理
Node API:      /api/v1/node/*       ← resolver 心跳、配置拉取/ACK、日志上报（原 Agent API）
Internal API:  /api/v1/internal/*    ← 跨进程调用，Shared Token 鉴权
Public Config: /api/v1/build/*      ← 静态资源，安装脚本下载
```

> **历史路径已废弃**：`/api/v1/member/*` → `/api/v1/user/*`；`/api/v1/agent/*` → `/api/v1/node/*`
...
```text
触发条件：心跳响应中携带 latest_config_version 数字
比较方式：resolver 主动比较 local_version < latest_config_version
不再使用：should_pull_config 布尔值字段
```
...
- dns_nodes              ← resolver 节点（含 region 字段区分类型）
