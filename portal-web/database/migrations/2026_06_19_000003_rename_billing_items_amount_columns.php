...
return new class extends Migration {
    public function up(): void
    {
        if (Schema::hasColumn('billing_items', 'amount') && ! Schema::hasColumn('billing_items', 'amount_minor')) {
            DB::statement('ALTER TABLE billing_items RENAME COLUMN amount TO amount_minor');
        }
        if (Schema::hasColumn('billing_items', 'unit_price') && ! Schema::hasColumn('billing_items', 'unit_price_minor')) {
            DB::statement('ALTER TABLE billing_items RENAME COLUMN unit_price TO unit_price_minor');
        }
    }
    ...
    public function down(): void
    {
        if (Schema::hasColumn('billing_items', 'amount_minor')) {
            DB::statement('ALTER TABLE billing_items RENAME COLUMN amount_minor TO amount');
        }
        if (Schema::hasColumn('billing_items', 'unit_price_minor')) {
            DB::statement('ALTER TABLE billing_items RENAME COLUMN unit_price_minor TO unit_price');
        }
    }
};
