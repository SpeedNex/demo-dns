...
//
// 2026-06-30: 无 profileID 时返回 REFUSED，防止裸 UDP/TCP 53 路径串用其他用户规则。
func (e *Engine) MatchWithProfile(profileID, domain string) *Decision {
	if profileID == "" {
		// 无 profile 拒绝服务，防止跨用户规则串用
		return &Decision{Action: "REFUSED", Reason: "no_profile"}
	}
	e.mu.RLock()
	pe, ok := e.profileEngines[profileID]
	e.mu.RUnlock()
	if !ok {
		return &Decision{Action: "REFUSED", Reason: "profile_not_found"}
	}
