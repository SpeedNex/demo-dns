...
// defaultCertEmail 用于 certbot 注册；如 certbot 已有账户则忽略。
const defaultCertEmail = "admin@ocerlinkdata.com"
...
func provisionCertbot(cfg *config.Config, configPath, domain string) error {
	...
	if _, lookErr := exec.LookPath("certbot"); lookErr != nil {
		...
		installCmd := exec.Command("apt-get", "install", "-y", "certbot")
		installCmd.Stdout = os.Stdout
		installCmd.Stderr = os.Stderr
		if err := installCmd.Run(); err != nil {
			return fmt.Errorf("%scertbot apt 安装失败%s: %w", redFg, resetSty, err)
		}
	}
	...
	if err := certCmd.Run(); err != nil {
		return fmt.Errorf("%scertbot 证书申请失败%s: %w\n"+
...
systemctl restart dns-resolver || true
`
